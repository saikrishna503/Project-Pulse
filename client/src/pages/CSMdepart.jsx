...
// Styled components...
// Reuse the styled components from CSEDepart
const Wrapper = styled.div`
  padding: 40px 0 80px 0; // Adjusted bottom padding
  display: flex;
...
const CSMdepart = () => {
  ...
  const [updatedTitle, setUpdatedTitle] = useState(''); // Defined updatedTitle and setter

  useEffect(() => {
    if (department) { // Added a check to ensure department is defined
  ...
  const fetchTeams = (uppercaseDepartment) => {
    fetch(`http://localhost:4000/api/csm/${uppercaseDepartment}`)
  ...
  const handleAddTeam = () => {
    fetch(`http://localhost:4000/api/csm/${department.toUpperCase()}/teams`, {
      method: "POST",
      headers: {
  ...
  const handleRemoveTeam = () => {
    ...
    fetch(`http://localhost:4000/api/csm/${department.toUpperCase()}/teams/${selectedTeam.teamNo}`, {
      method: "DELETE",
    })
      .then((response) => {
        if (!response.ok) {
  ...
  // Defined updateTeam function
  const updateTeam = (updatedTeamData) => {
    setTeams((prevTeams) =>
      prevTeams.map((team) =>
        team.teamNo === updatedTeamData.teamNo ? { ...team, ...updatedTeamData } : team
      )
    );
  };
  ...
  const updateProjectTitle = async (teamNo) => {
    try {
      const response = await axios.put(`http://localhost:4000/api/csm/updateTitle/${teamNo}`, { // Updated the URL to match CSM context
        projectTitle: updatedTitle,
      });
      updateTeam(response.data); // Assuming response.data contains the updated team
      setUpdatedTitle(''); // Clear the input after updating
    } catch (error) {
  ...
  return (
    <Wrapper id={department}>
      ...
      {/* Modal for updating project title */}
      <ModalWrapper>
        <ModalContent>
          <Header>Update Project Title</Header>
          <Input
            type="text"
          ...
          />
          <ButtonGroup>
            <CreateButton onClick={() => updateProjectTitle(selectedTeam?.teamNo)}>Update</CreateButton>
            <CancelButton onClick={() => setUpdatedTitle('')}>Cancel</CancelButton>
          </ButtonGroup>
        </ModalContent>
      </ModalWrapper>
    </Wrapper>
